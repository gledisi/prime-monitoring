...
#define NUM_CLIENTS_TO_EMULATE 100
...
void Usage(int argc, char **argv) {
    ...
    while (--argc > 0) {
        ...
        if ((argc > 1) && (!strncmp(*argv, "-l", 2))) {
            ...
            sscanf(argv[1], "%d", &tmp);
            Alarm(PRINT, "Clients to emulate %d\n", clients_number);
            if (tmp > clients_number) {
                clients_number = tmp;
            }
            argc--;
            argv++;
...
void Send_Update(const char *key, int value) {
    ...
    int ret;
    Alarm(PRINT, "Number of clients: %d", clients_number);
    if (num_outstanding_updates < clients_number) {
